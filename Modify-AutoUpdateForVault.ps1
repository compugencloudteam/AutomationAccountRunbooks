...
$SiteRecoveryRunbookName = "Modify-AutoUpdateForVault"
...
function Invoke-InternalWebRequest($Uri, $Headers, $Method, $Body, $ContentType, [ref]$Result)
{
    ...
    do
    {
        try
        {
            $ResultObject = Invoke-WebRequest -Uri $UpdateUrl -Headers $Header -Method 'PATCH' -Body $InputJson  -ContentType "application/json" -UseBasicParsing
            ($Result.Value) += ($ResultObject)
            break
        }
        catch
...
try
{
    ...
    try {
            ...
            Get-Header ([ref]$Header) $AadAudience
            $Result = @()
            Invoke-InternalWebRequest -Uri $UpdateUrl -Headers $Header -Method 'PATCH' -Body $InputJson  -ContentType "application/json" -Result ([ref]$Result)
            $Result = $Result[0]
    ...
    while($JobsInProgressList.Count -ne 0)
    {
        ...
        ForEach($JobAsyncUrl in $JobsInProgressList)
        {
            try
            {
                ...
                elseif($JobState -ieq "Succeeded" -or $JobState -ieq "PartiallySucceeded" -or $JobState -ieq "CompletedWithInformation")
                {
